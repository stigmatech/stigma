...
export async function createTwentyLead(data: LeadData) {
 ...
 const priorityTag = data.leadScore !== undefined && data.leadScore >= 70 ? "🔴 PRIORITÉ HAUTE" : 
                    data.leadScore !== undefined && data.leadScore >= 40 ? "🟠 PRIORITÉ MOYENNE" : "🟢 PRIORITÉ STANDARD";

 const contextDetails = [
 `# ${priorityTag}`,
 `## Strategic Lead: ${data.service || 'Contact'}`,
 data.leadScore !== undefined ? `**Lead Score**: ${data.leadScore}/100` : '',
 `**Source**: Elite Diagnostic Suite`,
 `---`,
 `**Technical Profile**:`,
 data.tier ? `- **Diagnostic Tier**: ${data.tier.toUpperCase()}` : '',

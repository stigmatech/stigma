...
const courses: Record<string, { en: CourseData; fr: CourseData }> = {
 ...
 { hour: "12:00 - 13:00", title: "Lunch Break", description: "Lunch included via Uber Eats or Doordash.", isLunch: true },
 ...
 { hour: "12h00 - 13h00", title: "Pause Déjeuner", description: "Lunch offert via Uber Eats ou Doordash.", isLunch: true },
...
export const getCourseSlug = (key: string, lang: string): string => {
 const courseEntry = courses[key];
 if (!courseEntry) return key;
 const l = lang as "en" | "fr";
 return courseEntry[l]?.slug || key;
};

export const getCorrectCourseSlug = (slugOrKey: string, lang: string): string | null => {
 const l = lang as "en" | "fr";
 const otherLang = l === "en" ? "fr" : "en";
 
 // 1. Is it a direct course key?
 if (courses[slugOrKey]) {
  return courses[slugOrKey][l]?.slug || null;
 }
 
 // 2. Is it a slug in the opposite language?
 const courseByOtherSlug = Object.values(courses).find(c => c[otherLang]?.slug === slugOrKey);
 if (courseByOtherSlug) {
  return courseByOtherSlug[l]?.slug || null;
 }

 // 3. Is it already the correct slug?
 const courseByCurrentSlug = Object.values(courses).find(c => c[l]?.slug === slugOrKey);
 if (courseByCurrentSlug) {
  return slugOrKey;
 }

...
export async function proxy(request: NextRequest) {
 ...
 if (hasSessionCookie || isDashboard) {
    ...
    user = await Promise.race([
      ...
      new Promise<any>((resolve) => setTimeout(async () => {
        if (isDashboard) {
          console.warn(`⚠️ Middleware timeout: Supabase auth check for [${pathname}] exceeded 2000ms. Falling back to session check.`);
          const { data: { session } } = await supabase.auth.getSession();
          resolve(session?.user || null);
        } else {
          resolve(null);
        }
      }, isDashboard ? 2000 : 800))
    ]).catch(err => {

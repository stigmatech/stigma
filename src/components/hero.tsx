...
export function Hero({ lang, dictionary }: { lang: any; dictionary: any }) {
  ...
  return (
    <section className="relative min-h-[90vh] flex flex-col justify-center bg-slate-950 overflow-hidden selection:bg-white/10">
      ...
      <div className="absolute inset-0 pointer-events-none z-0 opacity-20">
        <div className="absolute top-[-10%] left-[-5%] w-[400px] h-[600px] bg-white/5 blur-[120px] rounded-none rotate-12" />
        <div className="absolute bottom-[20%] right-[5%] w-[500px] h-[700px] bg-white/5 blur-[140px] rounded-none" />
      </div>
      ...
      {/* SCAN LINE ANIMATION */}
      <div
        className="absolute top-0 left-0 w-full h-px bg-white/10 shadow-[0_0_15px_rgba(255,255,255,0.2)] z-20 animate-scan"
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full pt-32 pb-24">
        ...
        >
          ...
          {/* MASSIVE TITLE */}
          <h1 className="text-6xl sm:text-7xl lg:text-[7rem] xl:text-[9rem] font-display font-black tracking-tighter uppercase leading-[0.85] text-slate-400 mb-12">
          ...
          {/* DESCRIPTION */}
          <p className="text-xl md:text-2xl text-slate-400 font-light leading-relaxed mb-16 max-w-3xl tracking-tight">
            {heroData.description}
          </p>

          {/* CTAs */}
          <div className="flex flex-col sm:flex-row gap-6">
            <Button asChild size="lg" className="rounded-none px-12 py-8 text-[11px] uppercase tracking-[0.4em] font-black bg-white text-slate-950 hover:bg-slate-100 transition-all border-none shadow-2xl">
              <Link href={`/${lang}/mip`}>{heroData.ctaPrimary}</Link>
            </Button>
            <Button 
              variant="ghost" 
              size="lg" 
              className="rounded-none px-10 py-8 text-[11px] uppercase tracking-[0.4em] font-bold text-white/40 hover:text-white hover:bg-white/5 transition-all"

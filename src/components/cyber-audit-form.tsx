...
import { CyberAuditPdfReport } from "./cyber-audit-pdf-report";
import html2canvas from "html2canvas";
import { jsPDF } from "jspdf";
import { Download, Loader2, AlertTriangle, ShieldCheck, ChevronRight, Lock } from "lucide-react";
import { BookingSession } from "./booking-session";
import { isProfessionalEmail, isValidPhoneNumber, formatToE164 } from "@/lib/validation";

interface CyberAuditFormProps {
  lang: string;
  dictionary: any;
}

export function CyberAuditForm({ lang, dictionary }: CyberAuditFormProps) {
  ...
  if (!dictionary || !dictionary.cyberAudit) return null;

  const dict = dictionary.cyberAudit;
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.save(`stigma-cyber-audit-${leadData.company.replace(/\s+/g, '-').toLowerCase() || 'report'}.pdf`);
    } catch (error) {
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`${successUrl}?score=${score}&tier=${tier}&name=${leadData.firstName}&email=${leadData.email}&phone=${formattedPhone}&type=cyber`);
    ...
    <div className="w-full">
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          ...
          >
            <div className="inline-flex items-center gap-3 bg-blue-600/10 text-blue-600 text-[10px] font-black tracking-[0.4em] uppercase px-6 py-2 border border-blue-600/20">
            ...
            <Button 
              onClick={handleStart}
              className="bg-blue-600 hover:bg-slate-950 text-white rounded-none px-12 py-8 text-[11px] font-black uppercase tracking-[0.3em] transition-all hover:scale-105 shadow-2xl shadow-blue-100"
            >
              {dict.cta_start}
            </Button>
          </motion.div>
        ...
        {step >= 1 && step <= totalQuestions && (
          ...
          >
            ...
            <div className="space-y-10">
              <span className="text-blue-600 font-black text-[10px] uppercase tracking-[0.4em] block">{dictionary.common.audit.question} {step} / {totalQuestions}</span>
            ...
            <div className="grid grid-cols-1 gap-4">
              {questions[step - 1].options.map((option: any, idx: number) => (
                <button
                  ...
                  className="group relative text-left p-10 border border-slate-100 hover:border-blue-600 hover:bg-blue-600 transition-all duration-500 rounded-none overflow-hidden"
                >
        ...
        {step === totalQuestions + 1 && (
          ...
          >
            ...
            <form onSubmit={handleLeadSubmit} className="space-y-6 text-left bg-white p-10 border border-slate-100 shadow-2xl">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <Input 
                    ...
                    type="text" 
                    required 
                    className="rounded-none border-t-0 border-l-0 border-r-0 border-b-2 border-slate-200 focus:border-blue-600 h-12 text-lg font-black uppercase tracking-tighter"
              ...
              <Button 
                type="submit"
                className="w-full bg-blue-600 hover:bg-slate-950 text-white rounded-none h-16 font-black uppercase tracking-[0.3em] text-[11px] transition-all"
                disabled={isSubmitting || !turnstileToken}
              >
                {isSubmitting ? dictionary.common.audit.processing : (dict.lead_form?.cta || "Voir mes Résultats")}
              </Button>
            </form>
          </motion.div>
        )}
          ...
          >
                ...
                <div className="relative w-48 h-48 flex items-center justify-center">
                  <svg className="absolute inset-0 w-full h-full -rotate-90">
                    ...
                    <motion.circle
                      ...
                      animate={{ strokeDashoffset: 553 - (553 * (calculateTotalScore() / 100)) }}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 border-b border-slate-100 group-hover:bg-slate-50 transition-colors">
                      <div className="p-10 lg:p-12 border-r border-slate-100 relative">
                        <div className="flex gap-6 items-start">
                          ...
                          <div className="space-y-3">
                            <h4 className="font-black text-slate-400 text-[10px] uppercase tracking-[0.4em]">{dict.pdf_report.radar[catId]}</h4>
                            <p className="text-lg font-black text-slate-950 uppercase tracking-tighter leading-tight">
                              {dict.dashboard.gap_detected}
                            </p>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="flex gap-6 items-start">
                          ...
                          <div className="space-y-3">
                            ...
                            <p className="text-lg font-light text-slate-700 tracking-tight leading-snug">
                              {dict.dashboard.remedy_description}
                            </p>
                          </div>
                        </div>
            ...
            <div className="pt-12">
              <BookingSession 
                ...
                dictionary={{
                  ...
                  schedulerTitle: dict.dashboard.scheduler_title || dict.dashboard.booking_title,
                  schedulerDescription: dict.dashboard.scheduler_description || dict.dashboard.booking_desc,
                }}
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center pt-24 border-t border-slate-100 pb-24">
              <Button asChild size="lg" className="rounded-none px-12 py-8 text-[11px] uppercase tracking-[0.3em] font-black bg-slate-950 text-white hover:bg-slate-800 shadow-2xl">
                <Link href={`/${lang}/quote?service=cybersecurity`}>
                  {dictionary.common.audit.resolve_vulnerabilities}
                </Link>
              </Button>

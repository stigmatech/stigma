...
export default function ProductCard({ product, dict, priority = false, isAuthenticated = false }: ProductCardProps) {
 ...
 if (!isAuthenticated) {
 router.push(`/${dict?.lang || "en"}/client-login`);
 return;
 }
 ...
 setIsLoading(false);
  ...
  const detailHref = product.href && !product.href.includes("contact") 
    ? product.href 
    : `/${dict?.lang || "en"}/marketplace/${product.id}`;
 ...
 <div className="w-12 h-12 flex items-center justify-center bg-white/5 border border-white/10 uppercase font-bold text-white/20 ">
 ...
 <h3 className="text-2xl font-black text-white uppercase tracking-tighter leading-tight group-hover/link:text-transparent group-hover/link:bg-clip-text group-hover/link:bg-linear-to-r group-hover/link:from-white group-hover/link:to-slate-300 transition-all duration-500">
 {product.name}
 </h3>
 </Link>
 </div>

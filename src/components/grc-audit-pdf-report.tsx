...
interface GrcAuditPdfReportProps {
  score: number;
  tier: string;
...
export const GrcAuditPdfReport = React.forwardRef<HTMLDivElement, GrcAuditPdfReportProps>(
  ({ score, tier, answers, dict, lang, company, isDashboard, common }, ref) => {
    if (!dict || !dict.pdf_report) return null;

    const chartData = [
      { subject: dict.pdf_report.radar.rppr, A: answers.rppr || 0, fullMark: 4 },
      { subject: dict.pdf_report.radar.inventory, A: answers.inventory || 0, fullMark: 4 },
      { subject: dict.pdf_report.radar.consent, A: answers.consent || 0, fullMark: 4 },
      { subject: dict.pdf_report.radar.incident, A: answers.incident || 0, fullMark: 4 },
      { subject: dict.pdf_report.radar.pia, A: answers.pia || 0, fullMark: 4 },
      { subject: dict.pdf_report.radar.training, A: answers.training || 0, fullMark: 4 },
    ];
    ...
    return (
      ...
      >
        {isDashboard ? (
          ...
          <>
            {/* Header Section */}
            <div className="flex justify-between items-start border-b-4 border-indigo-600 pb-6 mb-8 mt-4">
          <div>
          ...
          <div className="text-right">
            ...
            <p className="text-lg font-black text-indigo-600 mt-1">{company || dict.pdf_report.organization_label}</p>
          </div>
        </div>
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-indigo-600 font-bold uppercase tracking-[0.2em] text-sm mt-2">
            {dict.pdf_report.subtitle}
          </p>
        </div>

        {/* Score & Tier Banner */}
        <div className="bg-indigo-50 border-l-8 border-indigo-600 p-8 rounded-lg mb-10 flex items-center justify-between shadow-sm">
          <div>
            <p className="text-xs font-black uppercase tracking-widest text-gray-500 mb-1">{dict.pdf_report.score_label}</p>
            <p className="text-5xl font-black text-indigo-600">{score.toFixed(0)}<span className="text-2xl text-gray-400">%</span></p>
          </div>
          <div className="text-right">
          ...
          <div className="grid grid-cols-1 gap-6">
            {Object.entries(answers).filter(([_, s]) => s <= 2).slice(0, 3).map(([id], idx) => (
              <div key={id} className="flex gap-4">
                <div className="w-12 h-12 rounded-full bg-indigo-100 flex items-center justify-center font-black text-indigo-600 shrink-0 border-2 border-indigo-200">{idx + 1}</div>
        ...
        <div className="mt-12 text-center pt-6 border-t border-gray-200">
          ...
          <p className="text-xs font-black text-indigo-600 uppercase tracking-widest mt-2">www.stigmatech.ca</p>
        </div>
      </>
    )}
  </div>
...
GrcAuditPdfReport.displayName = "GrcAuditPdfReport";

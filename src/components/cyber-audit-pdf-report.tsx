...
interface CyberAuditPdfReportProps {
  score: number;
  tier: string;
...
export const CyberAuditPdfReport = React.forwardRef<HTMLDivElement, CyberAuditPdfReportProps>(
  ({ score, tier, answers, dict, lang, company, isDashboard, common }, ref) => {
    if (!dict || !dict.pdf_report) return null;

    const chartData = [
      { subject: dict.pdf_report.radar.endpoint, A: answers.endpoint || 0, fullMark: 4 },
      { subject: dict.pdf_report.radar.iam, A: answers.iam || 0, fullMark: 4 },
      { subject: dict.pdf_report.radar.backup, A: answers.backup || 0, fullMark: 4 },
      { subject: dict.pdf_report.radar.phishing, A: answers.phishing || 0, fullMark: 4 },
      { subject: dict.pdf_report.radar.soc, A: answers.soc || 0, fullMark: 4 },
    ];
          ...
          <>
            ...
            <div className="bg-blue-50 border-l-8 border-blue-600 p-8 rounded-lg mb-10 flex items-center justify-between shadow-sm">
              <div>
                ...
                <p className="text-5xl font-black text-blue-600">{score.toFixed(0)}<span className="text-2xl text-gray-400">%</span></p>
              </div>
              <div className="text-right">
              ...
              <div className="grid grid-cols-1 gap-6">
                {Object.entries(answers).filter(([_, s]) => s <= 2).slice(0, 3).map(([id], idx) => (
                  <div key={id} className="flex gap-4">
                    <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center font-black text-blue-600 shrink-0 border-2 border-blue-200">{idx + 1}</div>
                    <div className="flex-1 pt-2">
                      <p className="text-sm font-bold text-gray-950 uppercase tracking-tight mb-1">{dict.recommendations[id]?.title || id}</p>
                      <p className="text-sm text-gray-600 leading-snug">
                        {dict.recommendations[id]?.remedy || common?.audit?.investigation_required || "Investigation and hardening required."}
                      </p>
                    </div>
                  </div>
                ))}
            ...
            <div className="mt-12 text-center pt-6 border-t border-gray-200">
              ...
              <p className="text-xs font-black text-blue-600 uppercase tracking-widest mt-2">www.stigmatech.ca</p>
            </div>
          </>
        )}
      </div>
...
CyberAuditPdfReport.displayName = "CyberAuditPdfReport";

...
export function QuoteRequestForm({ lang, initialService, initialSpecificNeeds, initialMessage }: QuoteRequestFormProps) {
 ...
 useEffect(() => {
    if (initialService) {
      ...
      const matched = services.find(s => s.id === initialService || s.label === initialService);
      setForm(prev => ({ 
        ...prev, 
        service: matched ? matched.label : initialService 
      }));
    }
 ...
 setLoading(true);
 setError(null);
    const allPotentialNeeds = getSpecificNeedsForService();
    const readableNeeds = form.specificNeeds.map(id => {
      const found = allPotentialNeeds.find(n => n.id === id);
    ...
    try {
      const res = await fetch("/api/send", {
        ...
        body: JSON.stringify({
          ...form,
          specificNeeds: readableNeeds, // Send human readable labels to email
 ...
 { id: "other", label: isFr ? "Autre / Stratégie Globale" : "Other / Global Strategy", icon: "strategy" },
  ...
  const getSpecificNeedsForService = () => {
    if (form.service.includes("IT") || form.service.includes("Informatique")) return [
      { id: "helpdesk", label: isFr ? "Support Technique (Helpdesk)" : "Technical Support (Helpdesk)" },
      { id: "managed_infra", label: isFr ? "Gestion des Serveurs & Réseau" : "Server & Network Management" },
      { id: "procurement", label: isFr ? "Achat de Matériel/Licences" : "Hardware/Software Procurement" },
      { id: "vcio", label: isFr ? "Directeur TI Virtuel (vCIO)" : "Virtual CIO (vCIO)" },
      { id: "continuity", label: isFr ? "Sauvegarde & Continuité d'Affaires" : "Backup & Business Continuity" }
    ];
    if (form.service.includes("Cyber")) return [
      { id: "audit", label: isFr ? "Audit de Sécurité" : "Security Audit" },
      { id: "edr", label: isFr ? "Détection et Réponse (EDR/XDR)" : "Detection & Response (EDR/XDR)" },
      { id: "awareness", label: isFr ? "Sensibilisation des employés" : "Employee Security Awareness" },
      { id: "compliance", label: isFr ? "Conformité (Loi 25, ISO 27001)" : "Compliance (Loi 25, ISO)" },
      { id: "pentest", label: isFr ? "Tests de Pénétration" : "Penetration Testing" }
    ];
    if (form.service.includes("IA") || form.service.includes("AI")) return [
      { id: "rpa", label: isFr ? "Automatisation des processus (RPA)" : "Process Automation (RPA)" },
      { id: "agents", label: isFr ? "Agents IA / Chatbots" : "AI Agents / Chatbots" },
      { id: "data", label: isFr ? "Analyse de données prédictive" : "Predictive Data Analysis" },
      { id: "training", label: isFr ? "Formation des employés à l'IA" : "Employee AI Training" },
      { id: "copilot", label: isFr ? "Outils Microsoft Copilot" : "Microsoft Copilot Tools" }
    ];
    return [
      { id: "other", label: isFr ? "Autre (Veuillez préciser à la fin)" : "Other (Please specify at the end)" }
    ];
  };
 ...
 form.specificNeeds.includes(need.id) ? "border-slate-950 bg-slate-950 text-white" : "border-slate-100 hover:border-slate-950 bg-white"
 )}
 >
 <span className={cn(
 "font-black text-base uppercase tracking-tight transition-colors",
 form.specificNeeds.includes(need.id) ? "text-white" : "text-slate-500 group-hover:text-slate-950"
 )}>
 {need.label}
 </span>
 <div className={cn(
 "w-6 h-6 border flex items-center justify-center transition-all duration-500",
 form.specificNeeds.includes(need.id) ? "border-amber-500 bg-amber-500" : "border-slate-200 bg-slate-50 group-hover:border-slate-950"
 )}>
 {form.specificNeeds.includes(need.id) && <span className="material-symbols-outlined text-slate-950 text-[18px] font-black">check</span>}
 </div>
 </button>
 ))}
 </div>

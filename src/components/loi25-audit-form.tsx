...
import { Loi25AuditPdfReport } from "./loi-25-audit-pdf-report";
...
interface Loi25AuditFormProps {
  lang: string;
  dictionary: any;
}

export function Loi25AuditForm({ lang, dictionary }: Loi25AuditFormProps) {
  ...
  if (!dictionary || !dictionary.loi25Audit) return null;

  const dict = dictionary.loi25Audit;
  ...
  const generatePDF = async () => {
    ...
    try {
        ...
        onclone: (clonedDoc) => {
          ...
          for (let i = 0; i < elements.length; i++) {
            ...
            if (el.style) {
              ...
              el.style.setProperty('--primary', '#10b981', 'important'); // Emerald-500
      ...
      pdf.save(`stigma-loi25-audit-${leadData.company.replace(/\s+/g, '-').toLowerCase() || 'report'}.pdf`);
    } catch (error) {
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`${successUrl}?score=${score}&tier=${tier}&name=${leadData.firstName}&email=${leadData.email}&phone=${formattedPhone}&type=loi25`);
    ...
    <div className="w-full">
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          ...
          >
            <div className="inline-flex items-center gap-3 bg-emerald-600/10 text-emerald-600 text-[10px] font-black tracking-[0.4em] uppercase px-6 py-2 border border-emerald-600/20">
            ...
            <Button 
              onClick={handleStart}
              className="bg-emerald-600 hover:bg-slate-950 text-white rounded-none px-12 py-8 text-[11px] font-black uppercase tracking-[0.3em] transition-all hover:scale-105 shadow-2xl shadow-emerald-100"
            >
              {dict.cta_start}
            </Button>
          </motion.div>
        ...
        {step >= 1 && step <= totalQuestions && (
          ...
          >
            ...
            <div className="space-y-10">
              <span className="text-emerald-600 font-black text-[10px] uppercase tracking-[0.4em] block">{dictionary.common.audit.question} {step} / {totalQuestions}</span>
              <h2 className="text-4xl md:text-6xl font-display font-black text-slate-950 tracking-tighter leading-none uppercase">
                {questions[step - 1].question}
              </h2>
              <p className="text-slate-500 text-lg font-light tracking-tight max-w-3xl">
            ...
            <div className="grid grid-cols-1 gap-4">
              {questions[step - 1].options.map((option: any, idx: number) => (
                <button
                  ...
                  className="group relative text-left p-10 border border-slate-100 hover:border-emerald-600 hover:bg-emerald-600 transition-all duration-500 rounded-none overflow-hidden"
                >
        ...
        {step === totalQuestions + 1 && (
          ...
          >
            ...
            <form onSubmit={handleLeadSubmit} className="space-y-6 text-left bg-white p-10 border border-slate-100 shadow-2xl">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <input 
                    ...
                    type="text" 
                    required 
                    className="w-full rounded-none border-t-0 border-l-0 border-r-0 border-b-2 border-slate-200 focus:border-emerald-600 h-12 text-lg font-black uppercase tracking-tighter bg-transparent outline-none transition-colors"
              ...
              <Button 
                type="submit"
                className="w-full bg-emerald-600 hover:bg-slate-950 text-white rounded-none h-16 font-black uppercase tracking-[0.3em] text-[11px] transition-all"
        ...
        {step === totalQuestions + 2 && (
          ...
          >
            {/* 1. Header Scoreboard */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-center bg-slate-950 p-12 lg:p-20 border border-white/5 shadow-3xl relative overflow-hidden group rounded-none">
              <div className="absolute inset-0 bg-linear-to-br from-emerald-600/10 to-transparent pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
              ...
              <div className="lg:col-span-2 space-y-6 text-center lg:text-left">
                <div className="inline-block bg-emerald-600 text-white text-[9px] font-black px-4 py-1 uppercase tracking-[0.4em] mb-4">
            ...
            <div className="space-y-12">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-10">
                <div className="space-y-4">
                  ...
                  <h3 className="text-4xl font-display font-black text-slate-950 uppercase tracking-tighter">Bilan de Remédiation</h3>
                </div>
                <div className="flex gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-none bg-emerald-600" />
                ...
                {getHighPriorityGaps().map((catId, idx) => (
                  ...
                  >
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 border-b border-slate-100 group-hover:bg-slate-50 transition-colors">
                      ...
                      <div className="p-10 lg:p-12 bg-emerald-50/10 group-hover:bg-emerald-600/5 transition-colors relative">
                        <div className="flex gap-6 items-start">
                          <ShieldCheck className="w-6 h-6 text-emerald-600 shrink-0 mt-1" />
                          <div className="space-y-3">
                            <h4 className="font-black text-emerald-600 text-[10px] uppercase tracking-[0.4em]">{dict.dashboard.remedy_label}</h4>
                            <p className="text-lg font-light text-slate-700 tracking-tight leading-snug">
                              {dict.recommendations[catId].remedy}
                            </p>
                          </div>
                        </div>
                        <ChevronRight className="absolute right-8 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-200 group-hover:text-emerald-600 transition-colors" />
                      </div>
                    </div>
                  </motion.div>
                ))}
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center pt-24 border-t border-slate-100 pb-24">
              <Button asChild size="lg" className="rounded-none px-12 py-8 text-[11px] uppercase tracking-[0.3em] font-black bg-slate-950 text-white hover:bg-slate-800 shadow-2xl">
                <Link href={`/${lang}/quote?service=loi25`}>

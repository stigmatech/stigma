...
export async function POST(req: NextRequest) {
 try {
  ...
  // Handle PME Checkout Funnel specifically
  if (body.type === 'pme_order') {
    const { company, employees, plan, addons, lang } = body;
    const isFr = lang === 'fr';

    const { data, error } = await resend.emails.send({
      from: 'Stigma Technologies <onboarding@resend.dev>',
      to: ['fleurykoyo@stigmatech.ca'],
      subject: `[COMMANDE PME] ${company.name} - Plan ${plan.name}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 24px; background: #f9f9f9;">
          <div style="background: #0a0f2c; padding: 32px; text-align: center; margin-bottom: 24px;">
            <h1 style="color: white; margin: 0; font-size: 24px; letter-spacing: 4px; font-weight: 900; text-transform: uppercase;">NOUVELLE COMMANDE PME</h1>
            <p style="color: #2563eb; margin: 8px 0 0; font-size: 14px; font-weight: bold; text-transform: uppercase; letter-spacing: 2px;">Tunnel de Vente Actif</p>
          </div>

          <div style="grid-template-columns: 1fr 1fr; gap: 20px; display: grid;">
            <div style="background: white; padding: 24px; border: 1px solid #e5e7eb; border-top: 4px solid #0a0f2c;">
              <h2 style="font-size: 12px; color: #6b7280; margin: 0 0 16px 0; text-transform: uppercase; letter-spacing: 2px; font-weight: 900;">Entreprise</h2>
              <p style="font-size: 18px; font-weight: 900; margin: 0 0 8px 0; color: #0a0f2c;">${company.name}</p>
              <p style="font-size: 14px; color: #4b5563; margin: 0;">${company.industry || 'Secteur non spécifié'}</p>
              <p style="font-size: 14px; color: #4b5563; margin: 4px 0;">${company.address || 'Adresse non spécifiée'}</p>
              <p style="font-size: 14px; color: #2563eb; margin: 0;">${company.website || ''}</p>
            </div>

            <div style="background: white; padding: 24px; border: 1px solid #e5e7eb; border-top: 4px solid #2563eb;">
              <h2 style="font-size: 12px; color: #6b7280; margin: 0 0 16px 0; text-transform: uppercase; letter-spacing: 2px; font-weight: 900;">Configuration</h2>
              <p style="font-size: 18px; font-weight: 900; margin: 0 0 8px 0; color: #0a0f2c;">PLAN ${plan.name.toUpperCase()}</p>
              <p style="font-size: 14px; color: #4b5563; margin: 0;">Utilisateurs: <strong>${employees.length}</strong></p>
              <p style="font-size: 14px; color: #4b5563; margin: 4px 0;">Total Mensuel: <strong>${(plan.price * employees.length).toFixed(2)}$</strong></p>
              ${addons.length > 0 ? `<p style="font-size: 12px; color: #059669; font-weight: bold; margin: 8px 0 0 0;">+ ${addons.map((a: any) => a.id).join(', ')}</p>` : ''}
            </div>
          </div>

          <div style="background: white; padding: 24px; border: 1px solid #e5e7eb; margin-top: 20px;">
            <h2 style="font-size: 12px; color: #6b7280; margin: 0 0 16px 0; text-transform: uppercase; letter-spacing: 2px; font-weight: 900;">Liste des Utilisateurs</h2>
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
              <thead>
                <tr style="text-align: left; border-bottom: 2px solid #0a0f2c;">
                  <th style="padding: 12px 8px;">Prénom</th>
              ...
              <tbody>
                ${employees.map((emp: any) => `
                  <tr style="border-bottom: 1px solid #f3f4f6;">
                    <td style="padding: 12px 8px; font-weight: bold;">${emp.firstName}</td>
                    <td style="padding: 12px 8px;">${emp.lastName}</td>
                    <td style="padding: 12px 8px; color: #2563eb;">${emp.email}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
          ...
          <div style="margin-top: 32px; text-align: center;">
            <a href="https://stigmatech.ca/admin/orders" style="display: inline-block; background: #0a0f2c; color: white; padding: 16px 32px; text-decoration: none; font-size: 12px; font-weight: 900; text-transform: uppercase; letter-spacing: 2px;">Gérer dans l'Admin</a>
          </div>
        </div>
      `,
    });
    ...
    await createTwentyLead({
      ...
      service: `COMMANDE: Plan ${plan.name}`,
      message: `Commande PME complétée via le tunnel de vente.\nTotal Utilisateurs: ${employees.length}\nPlan: ${plan.name}\nAddons: ${addons.map((a: any) => a.id).join(', ')}\n\nUtilisateurs:\n${employees.map((e: any) => `- ${e.firstName} ${e.lastName} (${e.email})`).join('\n')}`,
      companySize: `${employees.length} employés`,
    });

    return NextResponse.json({ success: true });
  }

  // Normal lead capture logic
  const {

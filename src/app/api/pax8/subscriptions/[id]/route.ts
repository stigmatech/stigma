...
export async function PATCH(req: Request, props: { params: Promise<{ id: string }> }) {
 ...
 // 2. Fetch Subscription Data (Internal or Pax8)
  let sub;
  let isInternal = false;

  // First check if it's an internal order from our database
  ...
  if (internalOrder && ["essentiel", "pro", "elite", "essential"].includes(internalOrder.product_id)) {
    isInternal = true;
    sub = {
      id: internalOrder.id,
      productId: internalOrder.product_id,
      productName: internalOrder.product_id.charAt(0).toUpperCase() + internalOrder.product_id.slice(1),
      quantity: internalOrder.quantity,
      companyId: profile.pax8_company_id, // We assume alignment
      startDate: internalOrder.created_at,
      billingTerm: 'Monthly'
    };
  } else {
    try {
      ...
      return NextResponse.json({ error: `Subscription not found: ${e.message}` }, { status: 404 });
    }
  }
  ...
  // Enforce Minimum 5 Seats for Support 360
  const isSupport360 = sub.productName.toLowerCase().includes("support 360") || 
                       ["essentiel", "pro", "elite", "essential"].includes(sub.productId.toLowerCase());
  
  if (isSupport360 && newQuantity < 5) {
    return NextResponse.json({ error: 'Minimum of 5 seats required for Support 360 plans.' }, { status: 400 });
  }
 ...
 let allowedToReduceInPax8 = false;
  ...
  if (isInternal) {
    ...
    actionMessage = `Mise à jour réussie : Le nombre de sièges pour votre forfait internal a été ajusté à ${newQuantity}.`;
  } 
  // =========================================================
 ...
 new_quantity: newQuantity
  ...
  // Fetch latest banked count to return to UI
  const { data: latestBanked } = await supabase
    .from('banked_licenses')
    .select('count')
    .eq('pax8_company_id', profile.pax8_company_id)
    .eq('product_id', sub.productId)
  ...
  // If we were allowed to reduce in Pax8 or if we increased, the sub.quantity is already updated in Pax8
  // but our local 'sub' object might have the old value.
  const finalQuantity = (isInternal || diff > 0 || (diff < 0 && allowedToReduceInPax8)) 
    ? newQuantity 
    : oldQuantity;
  ...
  return NextResponse.json({ 
    ...
    subscription: {
      ...
      bankedCount: latestBanked?.count || 0
    }
  });

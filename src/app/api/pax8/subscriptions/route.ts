...
export async function GET() {
 ...
 { status: 403 }
  ...
  // 4. Fetch Internal Plans from orders table
  const { data: internalOrders } = await supabase
    .from('orders')
    .select('*')
    .eq('user_id', user.id)
    .eq('status', 'active')
    .in('product_id', ["essentiel", "pro", "elite", "essential"]);

  const internalSubs = (internalOrders || []).map((order: any) => ({
    id: order.id,
    productId: order.product_id,
    productName: order.product_id.charAt(0).toUpperCase() + order.product_id.slice(1),
    quantity: order.quantity,
    status: 'Active',
    price: 0, // Placeholder or fetch from mapping
    billingTerm: 'Monthly',
  ...
  // 5. Combine and Sort
  const allSubscriptions = [...internalSubs, ...subscriptions];

  // 6. Fetch Banked Licenses from local DB
 ...
 return acc;
  ...
  // 7. Append banked counts to subscriptions
  const subWithBanked = allSubscriptions.map((sub: any) => ({
  ...sub,

...
async function getFeaturedCaseStudy(slug: string) {
    try {
        const query = `*[_type == "caseStudy" && slug.current == $slug][0]`;
        return await client.fetch(query, { slug });
    } catch (e) {
        console.error("Failed to fetch Sanity case study:", e);
        return null;
    }
}
...
export default async function SemanticSearchPage(props: {
    ...
    // Fetch dynamic case study
    const sanityCaseStudy = await getFeaturedCaseStudy("legal-semantic-search");
    
    // Mapping Sanity data or Fallback
    const featuredCaseStudy = {
        title: sanityCaseStudy?.title?.[lang] || (lang === 'fr' ? "Recherche Sémantique Intelligente pour le Domaine Juridique" : "Intelligent Semantic Search for Legal Discovery"),
        description: sanityCaseStudy?.description?.[lang] || (lang === 'fr' 
            ? "Un moteur de recherche vectoriel permettant aux avocats d'interroger des millions de documents multilingues en utilisant des concepts juridiques plutôt que des mots-clés." 
            : "A vector-based search engine enabling lawyers to query millions of multilingual documents using legal concepts rather than keywords."),
        stats: sanityCaseStudy?.stats?.map((s: any) => ({
            value: s.value,
            label: s.label?.[lang]
        })) || [
            { value: "40h → 12m", label: lang === 'fr' ? "Efficacité de recherche" : "Search efficiency" },
            { value: "95%", label: lang === 'fr' ? "Précision de pertinence" : "Relevance accuracy" }
        ],
        imageUrl: sanityCaseStudy?.heroImage ? urlFor(sanityCaseStudy.heroImage).url() : "/images/semantic-search-hero.png",
        href: `/${lang}/case-studies/${sanityCaseStudy?.slug?.current || "legal-semantic-search"}`
    };
    ...
    return (
        <div className="min-h-screen bg-white selection:bg-surface-dark selection:text-white pt-24">
            ...
            <main>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                                ...
                                <div className="p-8 lg:p-12 flex flex-col justify-center">
                                    ...
                                    <Button asChild className="bg-white text-surface-dark hover:bg-blue-50 w-full md:w-fit rounded-none px-8 py-6 uppercase tracking-wider text-xs font-bold transition-all group/btn">
                                        <a href={featuredCaseStudy.href} className="flex items-center gap-2">
                                            {lang === 'fr' ? "Lire le Rapport Complet" : "Read Full Report"}
                ...
                <BookingSection lang={lang} dictionary={dict.booking} />

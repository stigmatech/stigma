...
export default async function ManagedAiAgents(props: {
 ...
 <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-950 mb-4 block underline decoration-1 underline-offset-8">
 ...
 {plan.id === 'pro-agent' && (
 <div className="absolute top-0 right-0 bg-slate-400 text-white text-[8px] font-black px-3 py-1 uppercase tracking-widest -translate-y-full">
 ...
 <span className={`material-symbols-outlined text-sm mt-0.5 ${plan.id === 'pro-agent' ? 'text-slate-400' : 'text-slate-300'}`}>check_circle</span>
 <span className={`text-[11px] font-medium leading-tight tracking-tight ${plan.id === 'pro-agent' ? 'text-slate-300' : 'text-slate-600'}`}>
 {feature}
 </span>
 </div>
 ))}
 <Button asChild className={`rounded-none w-full py-6 text-[10px] uppercase tracking-[0.2em] font-black ${plan.id === 'pro-agent' ? 'bg-white text-slate-950 hover:bg-slate-100' : 'bg-slate-950 text-white hover:bg-slate-800 transition-all border-none'}`}>
 ...
 <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 blur-[120px] rounded-full"></div>
 <div className="relative z-10 max-w-2xl">
 <span className="inline-block border border-white/20 text-slate-400 text-[9px] font-black tracking-[0.4em] uppercase px-4 py-2 mb-8 backdrop-blur-3xl">
 {dict.audit.tag}
 </span>
 <h2 className="text-4xl lg:text-6xl font-display font-black text-white uppercase tracking-tighter mb-8 leading-none ">
 {dict.audit.title}
 </h2>
 <p className="text-xl text-slate-400 font-light leading-relaxed tracking-tight mb-0 ">

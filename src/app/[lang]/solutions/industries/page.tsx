...
export default async function Industries(props: {
 ...
 <div className="absolute top-0 left-0 w-1 h-0 bg-slate-950 group-hover:h-full transition-all duration-700"></div>
 ...
 <span className="material-symbols-outlined text-4xl text-slate-400 group-hover:text-slate-950 transition-colors">{sector.icon}</span>
 </div>
 <h3 className="text-3xl font-display font-black text-slate-950 uppercase tracking-tighter mb-4 leading-none">
    <Link href={`/${lang}/solutions/industries/${sector.title.toLowerCase().includes('avocats') || sector.title.toLowerCase().includes('law') ? 'avocats' : sector.title.toLowerCase().includes('clinique') || sector.title.toLowerCase().includes('health') ? 'cliniques' : 'manufacturiers'}`} className="hover:underline decoration-blue-600 underline-offset-4">
      {sector.title}
    </Link>
 </h3>
 <p className="text-slate-500 font-light tracking-tight mb-8">{sector.description}</p>

 {sector.caseStudy && (
 <Link href={`/${lang}/case-studies/${sector.caseStudy.slug}`} className="inline-flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.4em] text-slate-950 hover:text-slate-500 transition-colors">

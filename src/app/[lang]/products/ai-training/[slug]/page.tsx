...
import { getCourseData, getAllCourseParams, getCorrectCourseSlug } from "@/data/ai-training-courses";
...
export async function generateMetadata(props: {
 ...
 return {
  ...
  openGraph: {
   title: `${course.title} | ${category} | Stigma Technologies`,
   description: course.description,
   url: `https://stigmatech.ca/${lang}/products/ai-training/${slug}`,
   siteName: "Stigma Technologies",
   type: "website",
  },
  alternates: { canonical: `https://stigmatech.ca/${lang}/products/ai-training/${slug}` },
 };
}
...
export default async function CourseDetailPage(props: {
 ...
 if (!course) {
  const correctSlug = getCorrectCourseSlug(slug, lang);
  if (correctSlug && correctSlug !== slug) {
   redirect(`/${lang}/products/ai-training/${correctSlug}`);
  }
  notFound();
 }

 const isFr = lang === "fr";
 const tc = dictionary.training.courseDetails;
 ...
 <span className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-1">{tc.level}</span>
 <span className="text-white font-bold">{course.level}</span>
 </div>
 <div className="bg-white/5 border border-white/10 px-6 py-4 flex flex-col">
 <span className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-1">{tc.duration}</span>
 ...
 <a href="#contact">{tc.bookForMyTeam}</a>
 </Button>
 ...
 {isLunch && (
 <span className="text-[10px] font-bold text-blue-400 uppercase tracking-widest px-2 bg-white">{tc.bonAppetit}</span>
 )}
 </div>
 );
 })}
 ...
 <span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">{tc.recommendedPrereq}</span>
 </div>
 <p className="text-sm font-bold text-[#0b0c10] group-hover:text-blue-700">
 {tc.requiresFundamentals}
 ...
 <h3 className="text-sm font-bold uppercase tracking-widest mb-6 relative z-10">{tc.whoIsItFor}</h3>
 ...
 <CourseFormSection lang={lang} courseTitle={course.title} dictionary={dictionary} price={course.price} />
